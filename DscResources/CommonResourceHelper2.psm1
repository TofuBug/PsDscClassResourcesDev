class CommonResourceHelper {   
    ...
    hidden static [bool] Test_IsNanoServer() {
        $NanoServer = $false        
        if ([CommonResourceHelper]::Test_CommandExists('Get-ComputerInfo')) {
    ...
    #>
    static [void] New_InvalidOperationException([String] $Message, [System.Management.Automation.ErrorRecord] $ErrorRecord) {    
        if ($null -eq $Message) {
    ...
    static [hashtable] Get_LocalizedData([String] $ResourceName) { 
        ...
        $localizedStringFileLocation = Join-Path -Path $resourceDirectory -ChildPath ([commonresourcehelper]::UICulture)
